...
class OcrService : Service() {
    ...
    private val TIMEOUT_MS = 5000L // 5 Segundos para fechar se não achar nada
    ...
    // Memória de Comparação (Para não piscar a tela se for igual)
    ...
    private fun createControls() {
        controlsView = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL; setPadding(20, 20, 20, 20); visibility = View.GONE
    ...
    private fun createInfoCard() {
        infoCardView = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL; setPadding(30, 25, 30, 25); visibility = View.GONE 
    ...
    // --- LOOP LÓGICO AJUSTADO (5 SEGUNDOS) ---
    private fun startOcrLoop() {
        scope.launch(Dispatchers.IO) {
            while (isServiceRunning) {
                ...
                // Se não detectou nada por 5s, fecha
                if (!hasValidData && (System.currentTimeMillis() - lastValidReadTime > TIMEOUT_MS)) {
                    hideCard()
                }
                
                delay(5000) // LÊ A CADA 5 SEGUNDOS (O segredo do não-stress)
            }
        }
    }
    ...
    private fun analyzeScreen(rawText: String): Boolean {
        ...
        while (dm.find()) { 
            ...
            if (d < 200) frameDist += d // Soma partes da mesma tela, mas variáveis zeradas no inicio
        }
        ...
        if (framePrice > 0.0 && (frameDist > 0.0 || frameTime > 0.0)) {
            ...
            // Só atualiza se for diferente da última leitura CONFIRMADA
