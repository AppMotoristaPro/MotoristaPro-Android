...
class OcrService : Service() {
    ...
    // Configurações Padrão (Lidas do SharedPreferences)
    ...
    // Recebe atualizações de config do Site ou App
    ...
    // --- CÉREBRO OCR (Atualizado) ---
    private fun analyzeScreen(rawText: String): Boolean {
        // LIMPEZA E FILTROS DE RUÍDO (AQUI ESTÁ A MUDANÇA)
        var cleanText = rawText.lowercase()
            .replace(Regex("r\\$\\s*[0-9.,]+\\s*/\\s*km"), "") // Ignora R$00,00/km
            .replace(Regex("\\+\\s*r\\$\\s*[0-9.,]+\\s*inclu[íi]do"), "") // Ignora +R$00,00 incluído
        ...
        // VALIDAÇÃO (2 Tempos / 2 Distâncias)
