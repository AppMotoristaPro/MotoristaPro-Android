...
# --- OCR SERVICE (Anti-Static Logic + Compact UI) ---
ocr_service_content = """
...
class OcrService : Service() {
    ...
    // Controle de Estabilidade (Anti-Loop Infinito)
    private var lastPrice = 0.0
    private var lastDist = 0.0
    private var staticFrameCount = 0 // Contador de vezes que viu a mesma coisa
    ...
    private fun setupMiniGlassOverlay() {
        ...
        // --- CONTAINER MINI GLASS ---
        rootLayout = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            setPadding(25, 15, 25, 15) // Bem compacto
        ...
        // 1. TOPO: VALOR (Compacto mas visível)
        tvValorTopo = TextView(this).apply {
            text = "R$ --"
            textSize = 20f // Reduzido de 28
        ...
        // 2. MEIO: DADOS (Pequeno)
        tvDadosMeio = TextView(this).apply {
            text = "-- km • -- min"
            textSize = 12f // Reduzido de 14
            setTextColor(Color.LTGRAY)
            gravity = Gravity.CENTER
            setPadding(0, 2, 0, 5)
        }

        // 3. BAIXO: RESULTADOS (Destaque)
        tvResultadosBaixo = TextView(this).apply {
            text = "R$ --/km • R$ --/h"
            textSize = 14f // Reduzido de 18
    ...
    private fun startOcrLoop() {
        scope.launch(Dispatchers.IO) {
            while (isRunning) {
                ...
                if (image != null) {
                    try {
                        ...
                        if (task.isSuccessful) {
                            processingDone = analyzeScreen(task.result.text)
                        }
                    } catch (e: Exception) {
                ...
                // Lógica de Delay
                // Se achou algo novo ou relevante (processingDone = true), segura 5s
                // Se não achou (ou escondeu por ser estático), tenta rápido em 1s
                if (processingDone) {
                    delay(5000)
                } else {
                    delay(1000)
    ...
    private fun analyzeScreen(rawText: String): Boolean {
        // Variáveis locais para este frame
        var framePrice = 0.0
        var frameDist = 0.0
        var frameTime = 0.0
        ...
        // --- LÓGICA ANTI-LOOP INFINITO ---
        
        // Se achou dados de corrida
        if (framePrice > 0.0 && (frameDist > 0.0 || frameTime > 0.0)) {
            
            // Verifica se é IGUAL à última leitura
            if (isSameAsLast(framePrice, frameDist)) {
                staticFrameCount++
                
                // Se viu a mesma coisa 2 vezes seguidas (no intervalo de 5s + 1s, ou seja, persistente)
                // Assume que é um elemento estático (ex: Saldo) e ESCONDE
                if (staticFrameCount >= 2) {
                    hideWindow()
                    return false // Retorna false para rodar o loop rápido e verificar mudança
                } else {
                    // Ainda é a primeira repetição (dentro do timeout), mantem visivel
                    // Retorna true para manter o delay de 5s
                    return true 
                }
            } else {
                // É NOVO! (Preço mudou ou Distância mudou)
                staticFrameCount = 0 // Reseta contador
                ...
                return true // Pausa 5s para leitura
            }
        } else {
            // Não achou nada (Mapa limpo)
            staticFrameCount = 0
            // Resetar memória ao sair da tela para garantir que se a mesma corrida voltar (reabrir app), ela seja mostrada
            lastPrice = 0.0 
            lastDist = 0.0
            hideWindow()
            return false
...
print("--- UI Miniatura e Lógica Anti-Estática ---")
create_file("app/src/main/java/com/motoristapro/android/OcrService.kt", ocr_service_content)

print("\nExecute:")
print("1. git add .")
print("2. git commit -m 'Fix: Static Content Loop and Mini UI'")
print("3. git push")
