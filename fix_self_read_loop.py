...
# --- OCR SERVICE (Com Crop de Segurança) ---
ocr_service_content = """
...
class OcrService : Service() {
    ...
    // Memória para evitar repetição (Anti-Spam visual)
            ...
            while (isRunning) {
                ...
                if (image != null) {
                    try {
                        ...
                        image.close() // Libera buffer rápido

                        // --- CORTE DE SEGURANÇA (Crop) ---
                        // Cortamos os 35% superiores da imagem. 
                        // Isso remove a nossa janela flutuante da visão do OCR.
                        // Assim ele NUNCA lerá os próprios números.
                        val cropY = (screenHeight * 0.35).toInt()
                        val cropHeight = screenHeight - cropY
                        
                        // Validar dimensões para evitar crash
                ...
                // Se achou algo novo, espera 5s (janela visível).
                // Se não achou, espera 1s e tenta de novo.
                if (shouldWait) {
                    delay(5000)
                } else {
                    delay(1000)
    ...
    private fun analyzeScreen(rawText: String): Boolean {
        ...
        if (framePrice > 0.0 && (frameDist > 0.0 || frameTime > 0.0)) {
            ...
            // Verifica se é a MESMA leitura anterior
            if (isSameAsLast(framePrice, frameDist)) {
                ...
                return true // Primeira vez vendo repetido, mantém janela (espera delay)
            } else {
                // É NOVO!
                sameFrameCount = 0
            ...
            // Nada na tela (Corte funcionou!)
            sameFrameCount = 0
            // Não resetamos lastPrice imediatamente para evitar piscar se falhar um frame
            hideWindow()
            return false
        }
    }
...
print("--- Aplicando Corte de Segurança (Anti-Loop) ---")
create_file("app/src/main/java/com/motoristapro/android/OcrService.kt", ocr_service_content)

print("\nExecute:")
print("1. git add .")
print("2. git commit -m 'Fix: Self-Reading Loop via Crop'")
print("3. git push")
