...
# --- OCR SERVICE COM MÁSCARA CIRÚRGICA ---
ocr_service_content = """
...
class OcrService : Service() {
    ...
    // Configurações da Posição da Janela (Para mascarar)
    private val WINDOW_Y_POS = 80 // Y fixo da janela
    private val WINDOW_WIDTH_PCT = 0.90 // 90% da largura
    ...
    private fun setupMiniGlassOverlay() {
        ...
        params.y = WINDOW_Y_POS // Posição Fixa Conhecida
    ...
    private fun setupMediaProjection(code: Int, data: Intent) {
        try {
            ...
            // Meia resolução para processamento
            ...
            while (isRunning) {
                ...
                if (image != null) {
                    try {
                        ...
                        // 2. MÁSCARA CIRÚRGICA (Pinta de preto onde a janela está)
                        // Como o bitmap é metade da tela, dividimos as coordenadas por 2
                        val canvas = Canvas(originalBitmap)
                        val paint = Paint()
                        paint.color = Color.BLACK
                        paint.style = Paint.Style.FILL
                        
                        // Coordenadas da Janela (Ajustadas para a escala do Bitmap)
                        val winY = WINDOW_Y_POS / 2f
                        val winW = (screenWidth * WINDOW_WIDTH_PCT).toFloat() // Largura da janela no bitmap
                        val winH = 200f // Altura estimada segura da janela (em pixels do bitmap)
                        val winX = (screenWidth - winW) / 2f // Centralizado
                        
                        // Desenha o quadrado preto APENAS sobre a janela
                        canvas.drawRect(winX, winY, winX + winW, winY + winH, paint)

                        // 3. OCR na imagem mascarada
